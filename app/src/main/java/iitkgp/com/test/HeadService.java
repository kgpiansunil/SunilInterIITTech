...
public class HeadService extends Service implements TextToSpeech.OnInitListener {
	private WindowManager mWindowManager;
	private WindowManager mWindowManagerNext;
	private View mPointer;
	private View mScrollRight;
	private View mNext;
	...
	public void onCreate() {
		...
		mScrollRight = LayoutInflater.from(this).inflate(R.layout.scroll_right, null);
				...
				public void onDone(String utteranceId) {
					...
					try {
						if(utteranceId.equals("MyText")){
							if (mPointer.getParent()!=null) mWindowManager.removeView(mPointer);
							if (mScrollRight.getParent()!=null) mWindowManager.removeView(mScrollRight);
							handler.postDelayed(runnableSpeak, helpDelay);
						}
	...
	public void onDestroy() {
		...
		try {
			...
			if (mScrollRight != null) mWindowManager.removeView(mScrollRight);
...
//										Log.d("KEYWORDS POS x", String.valueOf(element.getCornerPoints()[1].x));
//										Log.d("KEYWORDS POS y", String.valueOf(element.getCornerPoints()[1].y));
										allKeywords.add(element.getText().toLowerCase());
										allPositions.add(Objects.requireNonNull(element.getCornerPoints())[1]);
									}
								}
							}
							for(Screen screen : MainActivity.getScreens()){
								if(!screen.matchScreen(allKeywords)) {
									if (screen.getBasescreen()!=null) {
										screen = screen.getBasescreen();
										if(!screen.matchScreen(allKeywords)) {
											continue;
										}
									}
...
//								int index = allKeywords.indexOf(mainKeyword.toLowerCase());
								...
								if(index !=- 1) {
									...
									else if (screen.getGesture()==1) {
										try {
											mWindowManager.addView(mScrollRight, params);
										} catch (Exception ignored) {
											mWindowManager.updateViewLayout(mScrollRight, params);
										}
									}
								}
